  ...
  async iniciarTramite(req: AuthRequest, res: Response) {
    try {
      ...
      try {
        ...
        const grupoNombre = ficha.grupo ? `grupo_${ficha.grupo.nombre}` : null;
        ...
        const processResult = await iniciarProcesoEnCamunda({
          ...
          businessKey,
          variables: {
            id_tramite: tramite.id_tramite, // Incluir id_tramite para que el orchestrator sepa que ya existe
            id_consultante: tramite.id_consultante,
            id_grupo: tramite.id_grupo,
            grupoNombre,
            num_carpeta: tramite.num_carpeta,
            estadoInicial: 'en_tramite',
            observaciones: tramite.observaciones || '',
            creado_por: req.user?.id || userId || null,
            rol_creador: req.user?.rol || null,
          },
        });
