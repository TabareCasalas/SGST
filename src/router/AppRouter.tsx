...
import { useTramiteFilters } from '../hooks/useTramiteFilters';
import DocumentManager from '../components/Forms/DocumentManager';
...
// Componente de trámites simple
const TestTramites: React.FC = () => {
  const { user, logout } = useAuthContext();
  const navigate = useNavigate();
  const { deleteTramite } = useAppStore();
  const { showSuccess } = useAppNotifications();
  const [showForm, setShowForm] = React.useState(false);
  const [showDocumentManager, setShowDocumentManager] = React.useState(false);
  const [selectedTramiteId, setSelectedTramiteId] = React.useState('');
  ...
  const handleManageDocuments = (tramiteId: string) => {
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                {hasActiveFilters ? 
                  `Mostrando ${filteredCount} de ${totalTramites} trámites` :
                  `${totalTramites} trámites en total`
                }
              </p>
            </div>
            <button
            ...
            {filteredTramites.map((tramite) => (
              <div key={tramite.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-gray-900">{tramite.id}</h3>
                  <div className="flex space-x-1">
                    <button
                      onClick={() => handleManageDocuments(tramite.id)}
                ...
                <div className="flex justify-between items-center">
                  ...
                  {tramite.priority && (
                    <span className={`inline-block px-2 py-1 text-xs rounded-full ${
                      tramite.priority === 'urgente' ? 'bg-red-100 text-red-800' :
                      tramite.priority === 'alta' ? 'bg-orange-100 text-orange-800' :
          ...
          {filteredTramites.length === 0 && (
            <div className="text-center py-8">
              {hasActiveFilters ? (
                <>
                  <p className="text-gray-500 mb-4">No se encontraron trámites con los filtros aplicados</p>
                  <button
