...
export function CambiarEstadoTramiteModal({ tramite, onClose, onSuccess }: CambiarEstadoTramiteModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await ApiService.updateTramite(tramite.id_tramite, updateData);
      if (result.accepted) {
        showToast(
          result.message || 'Solicitud enviada. El estado se actualizará cuando Camunda complete la transición.',
          'info'
        );
      } else {
